---
layout: page
icon: fas fa-info-circle
order: 5
---

# 你好，我是 xFeng

> 荣耀存于心，而非留于形。

## 👨‍💻 关于我

**硬件工程师** | **全栈开发者** | **开源爱好者**

我是一名热爱技术的硬件产品经理，专注于将硬件设计与软件开发相结合，打造优秀的跨平台产品。

---

## 🛠️ 技术栈

### 硬件领域
- **嵌入式开发**: ESP32, STM32, Arduino
- **硬件测试**: 信号完整性分析, 电源测试, PCB设计评审
- **电子设计**: 电路原理图设计, BOM优化, 供应商管理

### 软件领域
- **系统开发**: Rust + Tauri (跨平台桌面应用)
- **后端服务**: Go + Gin + gRPC (微服务架构)
- **前端开发**: React + TypeScript + Tailwind CSS
- **脚本工具**: Python (爬虫、自动化、数据分析)

---

## 🚀 开源项目

### [XingJu 星聚](https://github.com/xfengyin/XingJu)
跨平台聚合应用，支持音乐/视频/小说/漫画多源搜索
- **技术栈**: Rust + Tauri v2 + React + TypeScript
- **亮点**: 赛博朋克UI设计、性能优化、懒加载、测试框架
- **GitHub**: [查看源码](https://github.com/xfengyin/XingJu)

### [Kongming 孔明](https://github.com/xfengyin/kongming)
微服务架构框架，支持热更新与动态配置
- **技术栈**: Go + Gin + gRPC + Etcd
- **亮点**: 服务发现、负载均衡、配置中心、监控告警
- **GitHub**: [查看源码](https://github.com/xfengyin/kongming)

### [Zen](https://github.com/xfengyin/zen)
Python模块化执行框架，支持依赖图与隔离执行
- **技术栈**: Python 3.10+
- **亮点**: 依赖图解析、隔离执行环境、意图解析、LLM集成
- **GitHub**: [查看源码](https://github.com/xfengyin/zen)

---

## 📝 博客内容

这个博客主要分享：
- **硬件开发经验**: 电路设计、测试方法、供应链管理经验
- **软件开发实践**: Rust、Go、Python项目实战经验
- **项目管理心得**: 硬件产品从0到1的全流程管理
- **技术学习笔记**: 新技术的学习过程与踩坑记录

---

## 📫 联系我

- **GitHub**: [@xfengyin](https://github.com/xfengyin)
- **邮箱**: xfeng@example.com
- **博客**: [xfengyin.github.io](https://xfengyin.github.io)

---

> 感谢访问我的博客，希望能给你带来一些启发！

*最后更新: 2026年4月*